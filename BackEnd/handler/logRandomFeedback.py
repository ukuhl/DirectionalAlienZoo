...
class LogUserPaymentHandler(BasisRequestHandler):
    ...
    def __parse_request_body(self):
        ...
        prolific_id = self.args["prolificId"]
        if prolific_id is None:
            self.send_custom_error(400, "Missing field 'prolificId'")
            raise Exception()

        return user_id, prolific_id

    def post(self):
        # Parse data
        try:
            user_id, prolific_id = self.__parse_request_body()
        except:
            return

        # Log data
        if self.datamgr.log_user_payment(user_id, prolific_id) is False:
