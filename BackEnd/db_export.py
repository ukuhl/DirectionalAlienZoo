...
class DataMgr():
    ...
    def _get_users_groups(self):
        try:
            cur = self.db.cursor()
            cur.execute("SELECT userId, expGroup FROM users")
    ...
    def export_survey(self, file_out_survey="survey.csv", file_out_demographics="demographics.csv"):
        try:
            ...
            cur = self.db.cursor()
            cur.execute("SELECT userId, varAge1, varAge2, varAge3, varAge4, varAge5, varAge6, varAge7, varGender1, varGender2, varGender3, varGender4 FROM demographics")
            for row in cur.fetchall():
                for i in range(1, 8):
    ...
    def export_decrypted_prolific_ids(self, file_out="decryptedprolificIDs.csv"):
        try:
            data_userId = []
            data_prolificId = []
            ...
            cur.execute("SELECT userId, prolificId FROM users_payout")
            for row in cur.fetchall():
                data_userId.append(str(row[0]))
                data_prolificId.append(str(row[1]))

            private_key = load_key("private_key.bin")
            data_prolificId = [decrypt(pid, private_key) for pid in data_prolificId]

            df = pd.DataFrame({"userId": data_userId, "prolificId": data_prolificId})        
            df.to_csv(file_out, index=False)
    ...
    def export_everything(self):
        return self.export_performance() and self.export_reactionTimes() and self.export_attentionCheck() and self.export_survey() and self.export_decrypted_prolific_ids()
