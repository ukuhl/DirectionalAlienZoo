...
user_name = ""  # INSERT NAME OF YOUR CHOICE
                                # USER 'user_name'@'localhost' IDENTIFIED BY 'password' WILL BE CREATED
...
class DataMgr():
    ...
    def __init_database(self):
        self.db.cursor().execute("CREATE TABLE IF NOT EXISTS users (userId TEXT NOT NULL, expGroup INT NOT NULL)")
        ...
        self.db.cursor().execute("CREATE TABLE IF NOT EXISTS users_payout (userId TEXT NOT NULL, prolificId TEXT NOT NULL)")
        self.db.commit()

    def add_new_user(self, user_id, expGroup):
        try:
            db = self.__connect_to_database()
            db.cursor().execute("INSERT INTO users (userId, expGroup) VALUES(%s,%s)", (user_id, int(expGroup)))
            db.commit()
    ...
    def get_user_by_userId(self, user_id):
        try:
            ...
            cur = db.cursor()
            cur.execute("SELECT expGroup FROM users WHERE userId=%s", (user_id,))
            result = cur.fetchone()

            return {"userId": user_id, "expGroup": int(result[0])}
        except Exception as ex:
            print(ex)
            return None
    ...
    def log_questionnaire_answers(self, user_id, question_id, checkbox_values):
        try:
            ...
            if question_id == -1:
                db.cursor().execute("INSERT INTO demographics (userId, varAge1, varAge2, varAge3, varAge4, varAge5, varAge6, varAge7, varGender1, varGender2, varGender3, varGender4) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                                    ...
                                    int(checkbox_values[7]), int(checkbox_values[8]), int(checkbox_values[9]), int(checkbox_values[10])))
    ...
    def log_user_payment(self, user_id, prolific_id):
        try:
            db = self.__connect_to_database()

            prolific_id = encrypt(prolific_id, self.public_key)   # Encrypt payment_id

            db.cursor().execute("INSERT INTO users_payout (userId, prolificId) VALUES(%s,%s)", (user_id, prolific_id))
            db.commit()
